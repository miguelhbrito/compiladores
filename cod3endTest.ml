...
let loop lexbuf resultado =
  let fornecedor = I.lexer_lexbuf_to_supplier (Lexico.token (Lexico.cria_estado ())) lexbuf in
...
let gera_nano num = 
   let cod = traduz ("exemplos/nano" ^ string_of_int(num) ^ ".py")
    in imprime_traducao cod
    
let gera_micro num = 
   let cod = traduz ("exemplos/micro" ^ string_of_int(num) ^ ".py")
    in imprime_traducao cod
